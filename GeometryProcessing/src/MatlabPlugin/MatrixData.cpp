...
void MatrixData::setDenseData( const void* const data, int nRows, int nCols, DataType type )
{
	clear();
	...
	int size = getByteCount(m_dataType)*m_nRows*m_nCols;
	if (!size)
		return;
...
void MatrixData::setSparseData(const void* const data, int* rowID, int* rowRange, 
							   int nRows, int nCols, int nRowID, int nRowRange, 
							   DataType type)
{
	clear();
	m_nRows = nRows; m_nCols = nCols;
	...
	int size = getByteCount(m_dataType) * m_nRowIDs;
	int rowIDSize = sizeof(int)*m_nRowIDs;
	int rowRangeSize = sizeof(int)*m_nColRanges;
	if (!size || !m_nRowIDs || !m_nColRanges)
		return;

	m_voidData = malloc(size);
	m_rowID    = (int*)malloc(rowIDSize);
	m_colRange = (int*)malloc(rowRangeSize);
	memcpy(m_voidData, data, size);
	memcpy(m_rowID, rowID, rowIDSize);
	memcpy(m_colRange, rowRange, rowRangeSize);
}

void MatrixData::setDenseDataPtr(void* const data, int nRows, int nCols, DataType type )
{
	clear();
	m_nRows = nRows; m_nCols = nCols;
	m_matType = MT_DENSE;
...
void MatrixData::setSparseDataPtr( void* const data, int* rowID, int* rowRange, int nRows, int nCols, int nRowID, int nRowRange, DataType type )
{
	clear();
	m_nRows = nRows; m_nCols = nCols;
...
bool MatrixData::sendToArray( mxArray*& m_matlabArray )
{
	...
	else if (getMatrixType() == MatrixData::MT_SPARSE)
	{
		...
		memcpy(mxGetIr(m_matlabArray), rID, nRID*sizeof(int));
		memcpy(mxGetJc(m_matlabArray), rRange, nRRange*sizeof(int));
	}
	return true;
}
...
bool MatrixData::getFromArray( mxArray* matlabArray )
{
	...
	int numDim = mxGetNumberOfDimensions(matlabArray);
	...
	if (!isSparse)
	{
		...
		int byteLength = row * col * MatrixData::getByteCount(dataType);
		if (!byteLength)
			return false;
	...
	else
	{
		...
		int * rowID= (int*)mxGetIr(matlabArray);
		int * rowRange=(int*)mxGetJc(matlabArray);
...
QDataStream& operator<<( QDataStream &dataStream, const MatrixData& mat )
{
	...
	{
		unsigned nBytes = mat.m_nRows * mat.m_nCols * elementSize;
		if (nBytes)
		{
	...
	{
		unsigned dataBytes = mat.m_nRowIDs * elementSize;
		unsigned rowIDBytes = mat.m_nRowIDs * sizeof(int);
		unsigned colRangeBytes = mat.m_nColRanges * sizeof(int);
...
QDataStream& operator>>( QDataStream &dataStream, MatrixData& mat )
{
	...
	else if (mat.m_matType == MatrixData::MT_SPARSE)
	{
		...
		if (dataBytes && rowIDBytes && colRangeBytes)
		{
			...
			mat.m_rowID    = new int[mat.m_nRowIDs];
			mat.m_colRange = new int[mat.m_nColRanges];
...
FieldData::FieldData( const QString& fieldName, int rows /*= 0*/, int cols /*= 0*/ )
{
...
bool FieldData::setElement( int ithRow, int ithCol, const QSharedPointer<AbstractData>& pData )
{
	if (!checkRange(ithRow, ithCol))
		return false;
	int idx = getIdx(ithRow, ithCol);
...
FieldData::ElementPtr FieldData::getElement( int ithRow, int ithCol )
{
	if (!checkRange(ithRow, ithCol))
		return ElementPtr();
...
#endif
void FieldData::reset( int rows, int cols )
{
	m_pDataArray.clear();
	if (rows <= 0 || cols <= 0)
		rows = cols = 0;
...
void FieldData::reset( const QString& name, int rows, int cols )
{
	setName(name);
	reset(rows, cols);
}
...
QDataStream& operator>>( QDataStream &dataStream, FieldData& s )
{
	s.clear();
	dataStream >> s.m_fieldName >> s.m_nRows >> s.m_nCols;
	int nElements = s.m_nRows * s.m_nCols;
